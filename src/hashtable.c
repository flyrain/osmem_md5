...
void test() {
	...
	printf("Done");
	printf("If we didnt do anything wrong..we should see %s", hashTableGet("k110"));
	install("phone", "9433120451");

	printf("Again if we go right, we have %s and %s", hashTableGet("k101"),
			hashTableGet("phone"));
//	displaytable();
	hashTableCleanup();
}
