...
/* for minix3.1.5 to minix 3.2.0
 get potential pgd page
 * return number of potential pgd*/
static int getPotentialPgd(int totalPageNumber, int pageSize, char *mem,
                    int pageIndex[totalPageNumber], int mem_size)
{
    ...
    for (i = 0; i < totalPageNumber; i++) {
        ...
        //for minix scan from beginning of pages
       unsigned pdeItem = *(unsigned *) ((unsigned) mem + startAddr + 0);
       unsigned pdeItem1 = *(unsigned *) ((unsigned) mem + startAddr + 4);

       /*
         second 4-byte in pgd
         c027 003b     3.1.5
         0187 0040     3.1.7
         01e7 0040     3.1.8
         0187 0040     3.2.0
       ...
       if (pdeItem == 0x1e7 && pdeItem1 <= 0x4001e7){
         pageIndex[i] = 1;
         pgdcount++;
         //         printf("0x%x\n", startAddr);
       }
...
/* for minix 3.2.1
 get potential pgd page
 * return number of potential pgd*/
static int getPotentialPgd_general(int totalPageNumber, int pageSize, char *mem,
                    int pageIndex[totalPageNumber], int mem_size)
{
    int pgdcount = 0;
    int i;
    for (i = 0; i < totalPageNumber; i++) {
        ...
        for (; j < 4 * 1024; j = j + 4) {
            ...
            puts("OLD_PGD_MATCH"); //not called here
#endif

            // for minix 3.2.1
            if ((pdeItem & 0x3) == 0x3  && (pdeItem & 0x418) == 0) {
                 /*
                 //check pte
              ...
              //too strong, but perfect for minix 3.2.1
              isPgd = 0;
              break;
            }
        }
...
unsigned getPgd(char *mem, int mem_size)
{
    ...
    if (gettimeofday(&earlier, NULL)) {
      ...
      pgdcount = getPotentialPgd_general(totalPageNumber, pageSize, mem, pageIndex,
                        mem_size);
