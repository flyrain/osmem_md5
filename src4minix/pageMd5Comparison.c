                    ...
                    unsigned pgd, int isWindows)
{
    ...
    if (isWindows)
        kernleStartAddr = KERNEL_START_ADDRESS;
    else
                      ...
                      unsigned offset)
{
    ...
    printf(" %08x %u", vaddr, offset / 4096);
    printf("\n");
                             ...
                             int pageSize, unsigned cr3Pages[])
{
    ...
    //    for (; startVirtual > startVirtualAddr - 1; startVirtual += 0x1000) {
    int loop_count = 0;
    for (; startVirtual <= 0xfffff000 && startVirtual >= 0; startVirtual += 0x1000) {
        if(loop_count > 0 && startVirtual  == 0 ) break;
        loop_count ++;
...
//              int allPhysicalPages = 0;
    ...
    int loop_count = 0;
    for (; vAddr <= 0xfffff000 && vAddr >= 0; vAddr += 0x1000) {
        if(loop_count > 0 && vAddr  == 0 ) break;
        loop_count ++;
        int rw = 0;             //read or write
        int us = 0;             //use or system
...
//        printf("vAddr 0x%x, pAddr 0x%x, us %d, g %d\n", vAddr, pAddr, us, g);
//          if (pAddr >= 0 && pAddr < mem->mem_size && us == 0 && g ==
//        256) {
        if (pAddr >= 0 && pAddr < mem->mem_size && g == 256) {
          //printf("pAddr 0x%x, vAddr 0x%x, rw %d, us %d, g %d, ps %d\n", pAddr, vAddr, rw, us, g, ps);
            (*allPages)++;
        }
...
void print_all_clusters(int cluster_len){
  puts("--------------------All clusters:----------------------------------");
  int i;
        ...
        unsigned cr3Pages[], int *kdi_time, int *allPages)
{
    ...
    //    print_all_clusters(cluster_index);
